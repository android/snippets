...
<body>
    ...
    <script>
        ...
        async function navigateToComponent(targetUrl, pushState = true) {{
            ...
            const fsBtn = document.getElementById('wasmFullscreenBtn');
            if (fsBtn) {{
                fsBtn.href = `../wasm.html?snippet=${{pageData.snippetId}}&standalone=true&density=1.0`;
            }}
            ...
            if (frame && frame.contentWindow) {{
                ...
                try {{
                    frame.contentWindow.location.hash = `/snippet/${{pageData.snippetId}}?theme=${{curTheme}}&preset=${{currentWasmPreset}}&seed=${{encodeURIComponent(currentWasmColor)}}`;
                }} catch (e) {{}}
                try {{
                    frame.contentWindow.postMessage(JSON.stringify({{
                        type: 'SELECT_SNIPPET',
